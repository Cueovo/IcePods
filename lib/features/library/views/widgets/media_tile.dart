...
class MediaTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Consolidated state so a screen reader hears what the colors show.
    final stateLabel = <String>[
      if (current) playing ? '正在播放' : '当前歌曲',
      if (item.isSong && item.requiresVip) 'VIP 歌曲',
      if (item.isCopyrightRestricted) '无版权',
      if (unavailable && !item.isCopyrightRestricted && !item.requiresVip)
        '无音源',
      if (canToggleMark && marked) '已收藏',
    ].join('，');
    return Semantics(
      button: true,
      selected: selected,
